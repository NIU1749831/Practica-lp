...
double primerTest2(MapaBase* mapaBase) {
	...
	if (mostraDetalls) {
		...
		std::cout << "Comment :=>> -------------" << std::endl;
		std::cout << "Comment :=>> Provem que tenim un unic shop amb acces per persones amb mobilitat reduida amb color 0xFA8072 a coordenades especifiques." << std::endl;
...
double  segonTest1(MapaBase* mapaBase) {
	...
	if (vecPdis.size() == 2) {
		...
		for (PuntDeInteresBase* pdi : vecPdis) {
			...
			if (isBotigaLatRight && isBotigaLonRight && pdi->getName() == "La Millor Pastisseria" && pdi->getColor() == 0xFA8072) {
				grade += 4;
...
double tercerTest1(MapaBase* mapaBase)
{
	...
	if (mostraDetalls) {
		...
		std::cout << "Comment :=>> \t El Viejo Roble 0x5D3FD3" << std::endl;
		std::cout << "Comment :=>> \t Nueva China amb color 0x00FFFF" << std::endl;
		...
		std::cout << "Comment :=>> \t El Colosseo amb color 0x7FFFD4 " << std::endl;
		std::cout << "Comment :=>> -------------" << std::endl;
	}
	...
	resultatMap.insert({ "El Viejo Roble", PdiResult { Coordinate{ 41.4902204, 2.1406477 }, 0x5D3FD3} });
	resultatMap.insert({ "Nueva China", PdiResult { Coordinate{ 41.4902107, 2.1411346 }, 0x00FFFF} });
	...
	resultatMap.insert({ "El Colosseo", PdiResult { Coordinate{ 41.4928550, 2.1426380 }, 0x7FFFD4} });
...
double quartTest1(MapaBase* mapaBase)
{
	...
	if (mostraDetalls) {
		...
		std::cout << "Comment :=>> Aquests son:" << std::endl;
		std::cout << "Comment :=>> \t UnSupermarket amb color 0xDFFF00" << std::endl;
		std::cout << "Comment :=>> \t UnTobacco amb color 0xFF7F50" << std::endl;
		std::cout << "Comment :=>> \t UnaBakery amb color 0xE85D75" << std::endl;
		std::cout << "Comment :=>> \t UnaBakeryAmbMobRedTancada amb color 0xFA8072" << std::endl;
		std::cout << "Comment :=>> \t UnaBakeryAmbMobRedOberta amb color 0x4CB944" << std::endl;
		std::cout << "Comment :=>> \t Un Shop restant amb color 0x4CBB17" << std::endl;
		std::cout << "Comment :=>> -------------" << std::endl;
	}
	...
	{
		if (pdi->getName() == "UnSupermarket" && pdi->getColor() == 0xDFFF00) {
			if (mostraDetalls)
				std::cout << "Comment :=>> ESTUDIANT HA TROBAT EL UNSUPERMARKET" << std::endl;
			grade += 4;
		}
		else if (pdi->getName() == "UnTobacco" && pdi->getColor() == 0xFF7F50) {
			if (mostraDetalls)
				std::cout << "Comment :=>> ESTUDIANT HA TROBAT UNTOBACCO" << std::endl;
			grade += 1;
		}

		else if (pdi->getName() == "UnaBakery" && pdi->getColor() == 0xFA8072) {
			if (mostraDetalls)
				std::cout << "Comment :=>> ESTUDIANT HA TROBAT UNABAKERY" << std::endl;
			grade += 1;
		}

		else if (pdi->getName() == "UnaBakeryAmbMobRedOberta" && pdi->getColor() == 0x4CBB17) {
			if (mostraDetalls)
				std::cout << "Comment :=>> ESTUDIANT HA TROBAT UNABAKERY PER MOB. REDUIDA OBERTA" << std::endl;
			grade += 2;
		}

		else if (pdi->getName() == "UnaBakeryAmbMobRedTancada" && pdi->getColor() == 0xFA8072) {
			if (mostraDetalls)
				std::cout << "Comment :=>> ESTUDIANT HA TROBAT UNABAKERY PER MOB. REDUIDA TANCADA" << std::endl;
			grade += 1;
		}

		else if (pdi->getName() == "UnaShopRestant" && pdi->getColor() == 0xFFEA00) {
			if (mostraDetalls)
